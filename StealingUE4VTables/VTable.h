...
/** Template for acquiring class method return type */
template <class Class, typename ... ParamTypes, typename ReturnType>
ReturnType MethodReturnType(ReturnType(Class::* MethodPtr)(ParamTypes ...) const);

//

template<class From, typename ReturnType, class Class, typename... ParamTypes>
struct TCopyQualifiersFromToFunction
{
	typedef ReturnType(Class::* Type)(ParamTypes...);
};

template<class From, typename ReturnType, class Class, typename... ParamTypes>
struct TCopyQualifiersFromToFunction<const From, ReturnType, Class, ParamTypes...>
{
	typedef ReturnType(Class::* Type)(ParamTypes...) const;
};

template<class From, typename ReturnType, class Class, typename... ParamTypes>
struct TCopyQualifiersFromToFunction<volatile From, ReturnType, Class, ParamTypes...>
{
	typedef ReturnType(Class::* Type)(ParamTypes...) volatile;
};

template<class From, typename ReturnType, class Class, typename... ParamTypes>
struct TCopyQualifiersFromToFunction<const volatile From, ReturnType, Class, ParamTypes...>
{
	typedef ReturnType(Class::* Type)(ParamTypes...) const volatile;
};

//

/** Type of class, owning the method using this macro. Doesn't remove CV. Convenient to use within other macro  */
#define __ThisType__ TRemoveReference<TRemovePointer<decltype(this)>::Type>::Type

/**
 * Same as __ThisType__, bet removes CV. Convenient to use within other macro
 * @note Could be just 'ThisClass' for UObjects
 */
#define __PureThisType__ TRemoveCV<__ThisType__>::Type

/** Creates a pointer to desired overloaded method based on provided param types */
#define SelectOverload(FunctionName, ...)\
	(\
		TCopyQualifiersFromToFunction\
		<\
		__ThisType__,\
		decltype(MethodReturnType<__PureThisType__, __VA_ARGS__>(&__PureThisType__::FunctionName)),\
		__PureThisType__,\
		__VA_ARGS__\
		>::Type\
...
static_assert(offsetof(FVTableLocationChecker, Value) == sizeof(void*), "VTable location detection failure");
